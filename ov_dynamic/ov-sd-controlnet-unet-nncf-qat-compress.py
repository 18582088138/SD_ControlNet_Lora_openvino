...
def main():
    ...
    controlnet = ControlNetModel.from_pretrained(
        # "sd-controlnet-canny", torch_dtype=torch.float32).to("cuda:1")
        f"{hf_model_dir}/sd-controlnet-canny", torch_dtype=torch.float16).to(device)

    pipeline = StableDiffusionControlNetPipeline.from_pretrained(
        f"{hf_model_dir}/stable-diffusion-v1-5", controlnet=controlnet,
        ).to(device)
    ...
    if args.dataset_name is not None:
        ...
        dataset = load_dataset(
            ...
            path="pokemon-blip-captions",
            data_files = "data/train-00000-of-00001-566cc9b19d7203f8.parquet",
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                control_model_input = latent_model_input.to(weight_dtype)
                controlnet_prompt_embeds = encoder_hidden_states.to(weight_dtype)
                ...
                down_block_res_samples = [d.to(weight_dtype) for d in down_block_res_samples]
                mid_block_res_sample = mid_block_res_sample.to(weight_dtype)
                # print("==down_block_res_samples==",down_block_res_samples)
                # print("==mid_block_res_sample==",mid_block_res_sample)
                ...
                if args.use_kd:
                    with torch.no_grad():
                        orig_output = teacher_model(noisy_latents, timesteps, encoder_hidden_states,
                                                    down_block_additional_residuals=down_block_res_samples,
                                                    mid_block_additional_residual=mid_block_res_sample,
                                                    return_dict=False,
                                                    )[0]
    ...
    UNET_CONTROL_ONNX_PATH = Path(f"{onnx_model_dir}/unet_controlnet_int8.onnx")
    UNET_CONTROL_OV_PATH = Path(f"{ir_int8_model_dir}/unet_controlnet_int8.xml")
    ...
    if not UNET_CONTROL_OV_PATH.exists():
        if not UNET_CONTROL_ONNX_PATH.exists():
            ...
            del unet
        del pipe.unet
        ov_unet = convert_model(UNET_CONTROL_ONNX_PATH)
        ...
        del ov_unet
        print(f'Unet with controlnet successfully converted to IR {UNET_CONTROL_OV_PATH}')
    else:
        del pipe.unet
    ...
    # export_pipeline = StableDiffusionControlNetPipeline(
    #     text_encoder=text_encoder,
    #     vae=vae,
    #     unet=export_unet,
    #     tokenizer=tokenizer,
