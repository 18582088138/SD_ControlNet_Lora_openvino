...
CONTROLNET_ONNX_PATH = Path(f'{onnx_dir}/controlnet-canny.onnx')
...
UNET_CONTROL_ONNX_PATH = Path(f'{onnx_dir}/unet-controlnet/unet-controlnet.onnx')
UNET_CONTROL_OV_PATH = UNET_CONTROL_ONNX_PATH.parents[1] / 'unet-controlnet.xml'
...
if not UNET_CONTROL_OV_PATH.exists():
    if not UNET_CONTROL_ONNX_PATH.exists():
        ...
        del unet
    del pipe.unet
    ov_unet = convert_model(UNET_CONTROL_ONNX_PATH, compress_to_fp16=True)
...
pipe = StableDiffusionControlNetPipeline.from_pretrained("stable-diffusion-v1-5", controlnet=controlnet)
UNET_ONNX_PATH = Path(f'{onnx_dir}/unet/unet.onnx')
...
if not UNET_OV_PATH.exists():
    ...
    del ov_unet
    print('Unet without controlnet successfully converted to IR')
else:
    del pipe.unet
    print(f"Unet without controlnet will be loaded from {UNET_OV_PATH}")



# ==== convert SD-text_encoder model to IR ====
TEXT_ENCODER_ONNX_PATH = Path(f'{onnx_dir}/text_encoder.onnx')
...
VAE_DECODER_ONNX_PATH = Path(f'{onnx_dir}/vae_decoder.onnx')
