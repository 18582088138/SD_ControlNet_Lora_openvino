...
def main():
    ...
    pipeline = StableDiffusionPipeline.from_pretrained(
        f"{hf_model_dir}/stable-diffusion-v1-5"
        ).to(device)
    ...
    UNET_CONTROL_ONNX_PATH = Path(f"{onnx_model_dir}/unet_int8.onnx")
    UNET_CONTROL_OV_PATH = Path(f"{ir_int8_model_dir}/unet_int8.xml")
    ...
    if not UNET_CONTROL_OV_PATH.exists():
        ...
        del ov_unet
        print(f'Unet successfully converted to IR {UNET_CONTROL_OV_PATH}')
    else:
        del pipe.unet
        print(f"Unet will be loaded from {UNET_CONTROL_OV_PATH}")
